...
export default function ExitIntentPopup() {
  ...
  useEffect(() => {
    // Check if shown in this session to prevent re-triggering
    const shown = sessionStorage.getItem('exit_popup_shown');
    if (shown) {
      setHasShown(true);
      return;
    ...
    // Set timer for 45 seconds (45000ms)
      ...
      {isVisible && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-6">
          ...
          <motion.div
            ...
            animate={{ opacity: 1, scale: 1, y: 0 }}
            exit={{ opacity: 0, scale: 0.9, y: 20 }}
            className="relative w-full max-w-lg bg-white overflow-hidden shadow-2xl border border-gray-200"
          >
            <button 
              onClick={closePopup}
            ...
            <div className="p-8 lg:p-12">
              <div className="text-center mb-10">
                <span className="inline-block px-4 py-1 bg-blue-50 text-blue-600 text-[10px] font-bold uppercase tracking-widest mb-6">
                  Exclusive Offer
                </span>
                <h2 className="text-4xl lg:text-5xl font-serif font-bold text-gray-900 mb-4 leading-tight">
                  Don't Leave Yet, <span className="italic font-normal">Wait!</span>
                </h2>
                <h3 className="text-xl font-serif text-blue-600 font-bold mb-4">
                  Save Further 85% Off Coupon — Use Anytime
                </h3>
                <p className="text-gray-500 text-sm leading-relaxed">
                  Enter your details to have our offer saved and sent straight to your inbox.
              ...
              <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
                <div className="space-y-1">
                  <input
                    {...register('name')}
                    placeholder="Full Name *"
                    className="w-full bg-gray-50 border border-gray-200 px-5 py-4 focus:outline-none focus:border-blue-500 transition-colors font-serif text-lg"
                  />
                  {errors.name && <p className="text-red-500 text-[10px] font-bold ml-1">{errors.name.message}</p>}
                </div>
                ...
                <button
                  type="submit"
                  disabled={isSubmitting}
                  className="w-full bg-gray-900 text-white py-5 text-sm font-bold uppercase tracking-widest hover:bg-blue-600 transition-all duration-500 flex items-center justify-center gap-3 shadow-xl"
                >
                  {isSubmitting ? 'Processing...' : (
                    <>
                      Claim My Discount
